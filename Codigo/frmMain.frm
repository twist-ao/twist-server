...
'CHOTS | Contador para torneos
If Torneo_Activado Then
  MinutosParaTorneo = MinutosParaTorneo + 1
End If
...
If MinutosParaWs = MinutosWs - 1 Then
    Call SendData(SendTarget.ToAll, 0, 0, ServerPackages.dialogo & "Servidor> Worldsave en 1 minuto ..." & FONTTYPE_SERVER)
End If
...
If Torneo_Activado Then
    ...
    If Not Torneo_HAYTORNEO Then
        If MinutosParaTorneo = 1 Then
          Call SendData(SendTarget.ToAll, 0, 0, "Z98")
        End If
      
        If MinutosParaTorneo = 2 Then
          Call SendData(SendTarget.ToAll, 0, 0, "Z95")
          Call reinicializarVotacionesTorneo
          Torneo_Votacion_Abierta = True
        End If

        If MinutosParaTorneo = 3 Then
          Call contarVotosTorneo
          Call SendData(SendTarget.ToAll, 0, 0, ServerPackages.dialogo & "TorneosAuto> La votación ha finalizado. El torneo a realizarse será: " & getTipoTorneoString() & ". Puedes tipear /TORNEOS en todo momento para ver cuanto falta para el próximo evento." & FONTTYPE_TORNEOAUTO)
          Torneo_Votacion_Abierta = False
        End If

        If MinutosParaTorneo = 5 Then
          Call SendData(SendTarget.ToAll, 0, 0, "Z96")
        End If

        If MinutosParaTorneo = 9 Then
          Call SendData(SendTarget.ToAll, 0, 0, "Z97")
        End If
      
        If MinutosParaTorneo = 10 Then
          Call crearTorneo
        End If
    Else
        If MinutosParaTorneo = 11 And Torneo_CantidadInscriptos < Torneo_Cupo Then
          Call SendData(SendTarget.ToAll, 0, 0, ServerPackages.dialogo & "TorneosAuto> El cupo aun no ha sido alcanzado! Todavia puedes ingresar tipeando /PARTICIPAR para entrar al torneo " & getTipoTorneoString() & FONTTYPE_TORNEOAUTO)
        End If
      
        If MinutosParaTorneo = 12 And Torneo_CantidadInscriptos < Torneo_Cupo Then
          Call rearmarTorneo
        End If
    End If
End If
    ...
    If UserList(iUserIndex).ConnID <> -1 Then
      ...
      If UserList(iUserIndex).ConnIDValida And UserList(iUserIndex).flags.UserLogged Then
        If UserList(iUserIndex).flags.Muerto = 0 Then
          ...
          If UserList(iUserIndex).flags.Descansar Then
            ...
            End If
          ElseIf (UserList(iUserIndex).flags.Hambre = 0 And UserList(iUserIndex).flags.Sed = 0) Then
...
Private Sub tmrAparicion_Timer()
    Call TimerMinutosAparicion
End Sub
