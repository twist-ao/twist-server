    ...
    If Not NpcImpacto Then
        If SkillDefensa + SkillTacticas > 0 Then  'Evitamos división por cero
            ...
            If Rechazo = True Then
                ...
                Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "MUS" & UserList(UserIndex).char.CharIndex)
            End If
        End If
    End If
End If
...
GolpeCritico = RandomNumber(1, 8)
If GolpeCritico = 5 Then
    dañoCritico = Round(daño * 0.1, 0)
Else
    dañoCritico = 0
End If

Call SendData(SendTarget.ToIndex, UserIndex, 0, "U2" & daño)
If dañoCritico > 0 Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "U5" & dañoCritico)
End If
Call CalcularDarExp(UserIndex, NpcIndex, daño + dañoCritico)
Npclist(NpcIndex).Stats.MinHP = Npclist(NpcIndex).Stats.MinHP - (daño + dañoCritico)
...
If UserList(UserIndex).Invent.WeaponEqpObjIndex <> 0 Then
    If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).Pegadoble >= 1 Then
        ...
        Else
            dañoCritico = 0
        End If
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "U2" & daño + dañoCritico)
        If dañoCritico > 0 Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "U5" & dañoCritico)
        End If
        Call CalcularDarExp(UserIndex, NpcIndex, daño + dañoCritico)
...
If Npclist(NpcIndex).Stats.MinHP > 0 Then
    ...
    If PuedeApuñalar(UserIndex) Then
       Call DoApuñalar(UserIndex, NpcIndex, 0, daño + dañoCritico)
...
If Npclist(NpcIndex).guerra.enGuerra = True And UserList(UserIndex).guerra.enGuerra = True Then
    If Npclist(NpcIndex).Numero = NPC_CASA Then
        If UserList(UserIndex).guerra.team = Npclist(NpcIndex).guerra.team Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No puedes atacar a tu propia base" & FONTTYPE_GUERRA)
            Exit Sub
        End If

        If Not PuedeAtacarBaseGuerra(UserIndex, NpcIndex) Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "Debes destruir al menos una torre de defensa antes de atacar la base enemiga!" & FONTTYPE_GUERRA)
            Exit Sub
        End If
    ElseIf Npclist(NpcIndex).Numero = NPC_TORRE Then
        If UserList(UserIndex).guerra.team = Npclist(NpcIndex).guerra.team Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No puedes atacar a tu propia torre" & FONTTYPE_GUERRA)
            Exit Sub
        End If
    End If
End If
...
If IntervaloPermiteAtacar(UserIndex) Then
    ...
    'CHOTS | Mandamos el FX de la espada
    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "MUW" & UserList(UserIndex).char.CharIndex)
...
If UserList(VictimaIndex).Invent.EscudoEqpObjIndex > 0 Then
    ...
    If UsuarioImpacto = False Then
      ...
      'Se rechazo el ataque con el escudo
        ...
        'CHOTS | Mandamos el FX del escudo
        Call SendData(SendTarget.ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "MUS" & UserList(VictimaIndex).char.CharIndex)
      End If
    End If
End If
...
If UsuarioImpacto(AtacanteIndex, VictimaIndex) Then
    Call SendData(SendTarget.ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "CXF" & UserList(VictimaIndex).char.CharIndex & "," & FXSANGRE & ",0," & SND_IMPACTO)
...
GolpeCritico = RandomNumber(1, 10)
If GolpeCritico = 5 Then
    dañoCritico = Round(daño * 0.1, 0)
Else
    dañoCritico = 0
...
Call SendData(SendTarget.ToIndex, AtacanteIndex, 0, "N5" & Lugar & "," & (daño + dañoCritico) & "," & UserList(VictimaIndex).Name)
Call SendData(SendTarget.ToIndex, VictimaIndex, 0, "N4" & Lugar & "," & (daño + dañoCritico) & "," & UserList(AtacanteIndex).Name)
If dañoCritico > 0 Then
    Call SendData(SendTarget.ToIndex, VictimaIndex, 0, "U5" & dañoCritico)
    Call SendData(SendTarget.ToIndex, AtacanteIndex, 0, "U5" & dañoCritico)
End If
UserList(VictimaIndex).Stats.MinHP = UserList(VictimaIndex).Stats.MinHP - (daño + dañoCritico)
...
If UserList(AtacanteIndex).flags.Hambre = 0 And UserList(AtacanteIndex).flags.Sed = 0 Then
    ...
    If PuedeApuñalar(AtacanteIndex) Then
        Call DoApuñalar(AtacanteIndex, 0, VictimaIndex, (daño + dañoCritico))
...
If isUserLocked(AttackerIndex) And isUserLocked(VictimIndex) Then
    PuedeAtacar = True
    Exit Function
End If
...
'CHOTS | En guerra pueden atacar
If UserList(AttackerIndex).guerra.enGuerra Then
    PuedeAtacar = True
    Exit Function
End If

'CHOTS | En torneo auto no pueden atacar si no estan en duelo, ni tampoco atacar a su pareja
If UserList(AttackerIndex).flags.enTorneoAuto Then
    If Not UserList(AttackerIndex).flags.enDueloTorneoAuto Then
        Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "No puedes atacar hasta que empiece el duelo!" & FONTTYPE_TORNEOAUTO)
        PuedeAtacar = False
        Exit Function
    Else
        If UserList(AttackerIndex).torneoPareja = VictimIndex Then
            Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "No puedes atacar a tu pareja!" & FONTTYPE_TORNEOAUTO)
            PuedeAtacar = False
            Exit Function
        Else
            PuedeAtacar = True
...
If Npclist(NpcIndex).guerra.enGuerra = True And UserList(AttackerIndex).guerra.enGuerra = True Then
    If Npclist(NpcIndex).Numero = NPC_CASA Then
        If UserList(AttackerIndex).guerra.team = Npclist(NpcIndex).guerra.team Then
            Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "No puedes atacar a tu propia base" & FONTTYPE_GUERRA)
            PuedeAtacarNPC = False
            Exit Function
        End If

        If Not PuedeAtacarBaseGuerra(AttackerIndex, NpcIndex) Then
            Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "Debes destruir al menos una torre de defensa antes de atacar la base enemiga!" & FONTTYPE_GUERRA)
            PuedeAtacarNPC = False
            Exit Function
        End If
    ElseIf Npclist(NpcIndex).Numero = NPC_TORRE Then
        If UserList(AttackerIndex).guerra.team = Npclist(NpcIndex).guerra.team Then
            Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "No puedes atacar a tu propia torre" & FONTTYPE_GUERRA)
            PuedeAtacarNPC = False
            Exit Function
        End If
    End If
