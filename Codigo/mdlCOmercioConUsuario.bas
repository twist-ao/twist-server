...
Else
    ...
    Call SendData(SendTarget.ToIndex, Destino, 0, ServerPackages.dialogo & UserList(Origen).Name & " desea comerciar. Si deseas aceptar, presiona F8." & FONTTYPE_TALK)
...
If UserList(UserList(UserIndex).ComUsu.DestUsu).ComUsu.Acepto = False Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "El otro usuario aun no ha aceptado tu oferta." & FONTTYPE_TALK)
    Exit Sub
End If
...
If UserList(OtroUserIndex).ComUsu.Objeto = FLAGORO Then
    ...
    If UserList(OtroUserIndex).ComUsu.Cant > UserList(OtroUserIndex).Stats.GLD Then
        Call SendData(SendTarget.ToIndex, OtroUserIndex, 0, ServerPackages.dialogo & "No tienes esa cantidad." & FONTTYPE_TALK)
        TerminarAhora = True
    End If
Else
