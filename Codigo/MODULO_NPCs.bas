...
On Error GoTo errhandler
   ...
   MiNPC = Npclist(NpcIndex)
    ...
    'CHOTS | Torneo Destruccion
    If UserIndex <> 0 And UserList(UserIndex).flags.enTorneoAuto = True And Npclist(NpcIndex).Numero = Torneo_NPCDESTRUCCION Then
        Call MuereNpcTorre(UserIndex, NpcIndex)
    ...
    'CHOTS | Apariciones
    If UserIndex <> 0 And Npclist(NpcIndex).Numero = NPC_APARICION Then
        Call LogGM("APARICIONES", "Muere una Aparicion a manos de: " & UserList(UserIndex).Name, False)
        Call MuereAparicion(NpcIndex)
        Call AparicionTiraItems(MiNPC)
        Call NPCTirarOro(MiNPC, UserIndex)
        Call EnviarOro(UserIndex)
        Exit Sub
    End If
...
Dim CharIndex As Integer
    ...
    Else
        Call SendData(sndRoute, sndIndex, sndMap, ServerPackages.crearChar & Npclist(NpcIndex).char.Body & "," & Npclist(NpcIndex).char.Head & "," & Npclist(NpcIndex).char.Heading & "," & Npclist(NpcIndex).char.CharIndex & "," & X & "," & Y)
    End If
...
If FX Then
    Call SendData(SendTarget.ToNPCArea, nIndex, Map, "CXF" & Npclist(nIndex).char.CharIndex & "," & FXIDs.FXWARP & ",0," & SND_WARP)
End If
