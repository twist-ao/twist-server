...
Public Const MAXMIEMBROS As Byte = 12
...
Dim DummyString     As String
    ...
    'CHOTS | Quitamos las piedras de clan
    Call QuitarObjetos(PIEDRACLAN, 10, FundadorIndex)
...
Dim GuildIndex      As Integer
    ...
    GuildIndex = UserList(UserIndex).GuildIndex
    If GuildIndex = 0 Then Exit Sub
    'CHOTS | No enviamos mas las GuildNews
...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If UserList(UserIndex).Stats.ELV < 25 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        refError = "Para fundar un clan debes ser nivel 25 y tener 100 skills en liderazgo."
        Exit Function
    End If

    'CHOTS | Necesita 10 piedras de clan
    If Not TieneObjetos(PIEDRACLAN, 10, UserIndex) Then
        refError = "Necesitas 10 Piedras de Clan para poder crear un Clan."
        Exit Function
    End If
